...
def index():
    try:
        ...
        return render_template('landing-three.html',
...
@main.route('/about')
def about():
    return render_template('about.html', title='About InnoFund')
...
def leaderboard():
    ...
    projects = []
    for project_id, project in all_projects.items():
        if project.get('status') == 'active':  # Only include active projects
            project['id'] = project_id
            project['votes'] = len(project.get('votes', {})) if project.get('votes') else 0
            project['funds_raised'] = float(project.get('funds_raised', 0))
            project['creator_name'] = project.get('creator_name', 'Unknown')
            projects.append(project)
    
    # Sort by votes and funding
    projects_by_votes = sorted(projects, key=lambda x: x['votes'], reverse=True)
    projects_by_funding = sorted(projects, key=lambda x: x['funds_raised'], reverse=True)
    
    return render_template('leaderboard.html',
                         title='Project Leaderboard',
                         projects_by_votes=projects_by_votes,
                         projects_by_funding=projects_by_funding)
...
@main.route('/api/request_tokens', methods=['POST'])
@login_required
def request_tokens():
    try:
        ...
        # Get recipient's document to verify it exists
        recipient_doc = db.collection('users').document(recipient_id).get()
        if not recipient_doc.exists:
            print(f"Recipient document not found for ID: {recipient_id}")
        ...
        try:
            ...
            current_notifications = recipient_data.get('notifications', [])
            ...
            db.collection('users').document(recipient_id).update({
                'notifications': current_notifications
            })
            
            print(f"Successfully updated recipient's notifications")
            return jsonify({'success': True})

        except Exception as update_error:
            print(f"Error updating notifications: {str(update_error)}")
            import traceback
            traceback.print_exc()
            return jsonify({'success': False, 'error': 'Failed to send notification'}), 500

    except Exception as e:
        print("Error requesting tokens:", str(e))
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_notifications():
    try:
        print("Fetching notifications for user:", current_user.id)
        ...
        print("First notification:", notifications[0] if notifications else "No notifications")
...
@main.route('/api/handle_token_request', methods=['POST'])
@login_required
    ...
    try:
        ...
        if accept:
            ...
            sender_tx = {**transaction, 
                        'type': 'token_exchange_sent',
                        'recipient_id': current_user.id,
                        'recipient_name': current_user.display_name}
            
            db.collection('users').document(from_user_id).update({
                'transactions': firestore.ArrayUnion([sender_tx])
            })

            # Add transaction to recipient's history
            recipient_tx = {**transaction,
                          'type': 'token_exchange_received',
                          'sender_id': from_user_id,
                          'sender_name': notification['from_user_name']}
            
            user_doc.update({
                'transactions': firestore.ArrayUnion([recipient_tx])
            })

            # Create notification for sender about acceptance
            sender_notification = {
                'id': str(datetime.now().timestamp()),
                'type': 'token_request_accepted',
                'from_user_id': current_user.id,
                'from_user_name': current_user.display_name,
                'amount': amount,
                'timestamp': datetime.now().isoformat(),
                'read': False,
                'message': f'Your token request for {amount} AVAX has been accepted'
            }

            db.collection('users').document(from_user_id).update({
                'notifications': firestore.ArrayUnion([sender_notification])
            })
            ...
            decline_notification = {
                'id': str(datetime.now().timestamp()),
                'type': 'token_request_declined',
                'from_user_id': current_user.id,
                'from_user_name': current_user.display_name,
                'amount': notification['amount'],
                'timestamp': datetime.now().isoformat(),
                'read': False,
                'message': f'Your token request for {notification["amount"]} AVAX has been declined'
            }

            db.collection('users').document(notification['from_user_id']).update({
                'notifications': firestore.ArrayUnion([decline_notification])
            })

        # Update notifications in user's document (now with the processed notification removed)
        user_doc.update({
            'notifications': notifications
        })
        ...
        print("Handle token request error:", str(e))
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
...
def competitions():
    try:
        # Get research requests from Firestore
        competitions_ref = db.collection('competitions').order_by('created_at', direction=firestore.Query.DESCENDING)
        competitions = [doc.to_dict() | {'id': doc.id} for doc in competitions_ref.stream()]
        
        return render_template('competitions.html', 
                             title='Research Requests',
                             competitions=competitions)
    except Exception as e:
        print("Error loading research requests:", str(e))
        flash('Error loading research requests', 'error')
        return redirect(url_for('main.dashboard'))
...
def view_competition(competition_id):
    try:
        ...
        competition_doc = db.collection('competitions').document(competition_id).get()
        if not competition_doc.exists:
            flash('Research request not found', 'error')
            return redirect(url_for('main.competitions'))
            
        competition = competition_doc.to_dict() | {'id': competition_doc.id}
        
        # Get solutions for this research request
        solutions_ref = db.collection('competitions').document(competition_id).collection('solutions')
        solutions = [doc.to_dict() | {'id': doc.id} for doc in solutions_ref.stream()]
        
        # Sort solutions by votes (descending)
        solutions.sort(key=lambda x: x.get('votes', 0), reverse=True)
        
        # Check if user has already submitted a solution
        has_submitted = any(s.get('created_by') == current_user.id for s in solutions)
        ...
        if solutions:
            ...
            if user_doc.exists:
                user_data = user_doc.to_dict()
                user_votes = set(user_data.get('solution_votes', []))
        ...
        print("Error viewing research request:", str(e))
        flash('Error loading research request details', 'error')
        return redirect(url_for('main.competitions'))

@main.route('/api/create-competition', methods=['POST'])
@login_required
def create_competition():
    try:
        title = request.form.get('title')
        description = request.form.get('description')
        prize_amount = float(request.form.get('prize_amount', 0))
        
        if not all([title, description, prize_amount]) or prize_amount <= 0:
            return jsonify({'success': False, 'error': 'Invalid research request details'}), 400
            
        # Create research request document
        competition_data = {
            'title': title,
        ...
        if files:
            # Get the storage bucket using the initialized app
            bucket = storage.bucket(app=get_app('storage'))
            ...
            for file in files[:2]:  # Limit to 2 files
                if file and file.filename:
                    ...
                    filename = f"competitions/{uuid.uuid4()}{ext}"
                    ...
                    # Add file info to research request data
                    competition_data['files'].append({
                        'name': file.filename,
                        'url': blob.public_url
                    })
        
        # Store research request in Firestore
        db.collection('competitions').add(competition_data)
        
        return jsonify({'success': True})
        
    except Exception as e:
        print("Error creating research request:", str(e))
        return jsonify({'success': False, 'error': str(e)}), 500

@main.route('/api/competitions/<competition_id>/submit-solution', methods=['POST'])
@login_required
def submit_solution(competition_id):
    try:
        # Get research request
        competition_doc = db.collection('competitions').document(competition_id).get()
        if not competition_doc.exists:
            return jsonify({'success': False, 'error': 'Research request not found'}), 404
            
        competition = competition_doc.to_dict()
        if competition['status'] != 'active':
            return jsonify({'success': False, 'error': 'Research request is not active'}), 400
            
        # Check if user has already submitted
        solutions_ref = db.collection('competitions').document(competition_id).collection('solutions')
        existing_solution = solutions_ref.where('created_by', '==', current_user.id).limit(1).get()
        if len(list(existing_solution)) > 0:
        ...
        if not description and 'files[]' not in request.files:
            return jsonify({'success': False, 'error': 'Please provide either a description or files'}), 400
            ...
            if files:
                ...
                for file in files[:2]:  # Limit to 2 files
                    if file and file.filename:
                        # Create unique filename
                        ext = os.path.splitext(file.filename)[1]
                        filename = f"solutions/{competition_id}/{uuid.uuid4()}{ext}"
                        ...
                        # Add file info to solution data
                        solution_data['files'].append({
                            'name': file.filename,
                            'url': blob.public_url
                        })
        
        # Store solution in Firestore
        solutions_ref.add(solution_data)
        
        return jsonify({'success': True})
        
    except Exception as e:
        print("Error submitting solution:", str(e))
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500

@main.route('/api/competitions/<competition_id>/end', methods=['POST'])
@login_required
def end_competition(competition_id):
    try:
        # Get research request
        competition_doc = db.collection('competitions').document(competition_id)
        competition = competition_doc.get()
        ...
        # Check if user is the research request creator
        if competition_data['created_by'] != current_user.id:
            return jsonify({'success': False, 'error': 'Only the research request creator can end it'}), 403
            
        # Get solutions sorted by votes
        solutions_ref = competition_doc.collection('solutions')
        solutions = [doc.to_dict() | {'id': doc.id} for doc in solutions_ref.stream()]
        solutions.sort(key=lambda x: x.get('votes', 0), reverse=True)
        
        if solutions:
            # Get winner (solution with most votes)
            winner = solutions[0]
            winner_id = winner['created_by']
            
            # Create transaction records
            transaction = {
                'timestamp': datetime.now().isoformat(),
                'amount': competition_data['prize_amount'],
                'type': 'research_request_prize',
                'competition_id': competition_id,
                'competition_title': competition_data['title']
            }
            
            # Add transaction to winner's history
            winner_tx = {**transaction, 'type': 'research_request_prize_received'}
            db.collection('users').document(winner_id).update({
                'transactions': firestore.ArrayUnion([winner_tx])
            })
            
            # Add transaction to organizer's history
            organizer_tx = {**transaction, 'type': 'research_request_prize_paid'}
            db.collection('users').document(competition_data['created_by']).update({
                'transactions': firestore.ArrayUnion([organizer_tx])
            })
            
            # Create notification for winner
            winner_notification = {
                'id': str(datetime.now().timestamp()),
                'type': 'research_request_win',
                'competition_id': competition_id,
                'competition_title': competition_data['title'],
                ...
                'timestamp': datetime.now().isoformat(),
                'read': False,
                'message': f"Congratulations! Your solution '{winner['title']}' won the research request '{competition_data['title']}'! The prize of {competition_data['prize_amount']} AVAX has been automatically transferred to your account."
            }
            
            # Add notification to winner's notifications
            db.collection('users').document(winner_id).update({
                'notifications': firestore.ArrayUnion([winner_notification])
            })
            
            # Update research request status
            competition_doc.update({
                'status': 'completed',
                'winner_id': winner_id,
                'winner_name': winner['author_name'],
                'winning_solution_id': winner['id'],
                'ended_at': datetime.now().isoformat(),
                'prize_claimed_at': datetime.now().isoformat()
            })
            
        return jsonify({'success': True})
        
    except Exception as e:
        print("Error ending research request:", str(e))
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500

@main.route('/api/competitions/<competition_id>/accept-prize', methods=['POST'])
@login_required
def accept_competition_prize(competition_id):
    try:
        # Get research request
        competition_doc = db.collection('competitions').document(competition_id)
        competition = competition_doc.get()
        ...
        if competition_data['winner_id'] != current_user.id:
            return jsonify({'success': False, 'error': 'Only the winner can accept the prize'}), 403
        ...
        print("Error accepting research request prize:", str(e))
        return jsonify({'success': False, 'error': str(e)}), 500

@main.route('/api/competitions/<competition_id>/decline-prize', methods=['POST'])
@login_required
def decline_competition_prize(competition_id):
    try:
        # Get research request
        competition_doc = db.collection('competitions').document(competition_id)
        competition = competition_doc.get()
        ...
        # Verify user is the winner
        if competition_data['winner_id'] != current_user.id:
            return jsonify({'success': False, 'error': 'Only the winner can decline the prize'}), 403
            
        # Create refund notification for organizer
        refund_notification = {
            'id': str(datetime.now().timestamp()),
            'type': 'research_request_prize_declined',
            'competition_id': competition_id,
            'competition_title': competition_data['title'],
            'prize_amount': competition_data['prize_amount'],
            'from_user_id': current_user.id,
            ...
            'timestamp': datetime.now().isoformat(),
            'read': False,
            'message': f"The winner has declined the prize for research request '{competition_data['title']}'. {competition_data['prize_amount']} AVAX has been refunded to your account."
        }
        
        # Add notification to organizer's notifications
        db.collection('users').document(competition_data['created_by']).update({
            'notifications': firestore.ArrayUnion([refund_notification])
        })
        ...
        refund_tx = {
            'timestamp': datetime.now().isoformat(),
            'amount': competition_data['prize_amount'],
            'type': 'research_request_prize_refunded',
            'competition_id': competition_id,
            'competition_title': competition_data['title']
        }
        
        # Add refund transaction to organizer's history
        db.collection('users').document(competition_data['created_by']).update({
            'transactions': firestore.ArrayUnion([refund_tx])
        })
        
        # Update research request status
        competition_doc.update({
            'status': 'prize_declined',
            'prize_declined_at': datetime.now().isoformat()
        })
        
        return jsonify({'success': True})
        
    except Exception as e:
        print("Error declining research request prize:", str(e))
        return jsonify({'success': False, 'error': str(e)}), 500
